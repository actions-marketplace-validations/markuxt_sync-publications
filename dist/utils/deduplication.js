 ...
 */
const tokenizeCache = new WeakMap();
/**
 ...
 * interchangeably.
 */
export function _cachedTokens(obj) {
    let cached = tokenizeCache.get(obj);
    if (!cached) {
        cached = tokenize(obj.title);
        tokenizeCache.set(obj, cached);
    }
    return cached;
}
/**
 ...
 */
export function _resetTokenizeCache() {
...
export function isDuplicate(candidate, existing) {
    ...
    if (Math.abs(candidate.year - existing.year) > 1)
        return false;
